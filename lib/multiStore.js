...
class MultiStore {
  ...
  get(type, url) {
    return this.newStore.get(type, url).catch(error => { return this.oldStore.get(type, url); });
  }

  etag(type, url) {
    return this.newStore.etag(type, url).catch(error => { return this.oldStore.etag(type, url); });
  }
