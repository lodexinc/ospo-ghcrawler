...
class AmqpQueue {
  ...
  _reconnect() {
    ...
    if (this.channel) {
      this.logger.warn(`Reconnecting ${this.name} using AMQP at ${url}`);
    }
    ...
    this.channel = Q
      ...
      .then(
      channel => {
        this.logger.info(`Connected ${this.name} using AMQP at ${url}`);
        return channel;
      },
      error => {
        this.logger.warn(`Reconnection Failed for ${this.name} using AMQP at ${url}`);
