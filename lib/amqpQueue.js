...
class AmqpQueue {
  ...
  _reconnect() {
    ...
    if (this.channel) {
      this.logger.warn(`Reconnecting ${this.name} using AMQP`);
    }
    ...
    this.channel = Q
      ...
      .then(
      channel => {
        this.logger.info(`Connected ${this.name} using AMQP`);
        return channel;
      },
      error => {
        this.logger.warn(`Reconnection failed for ${this.name} using AMQP`);
