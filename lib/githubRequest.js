...
class GitHubRequest {
  ...
  buildOptions(options) {
    const headers = extend({}, GitHubRequest.defaultOptions.headers, options.headers);
    const result = extend({}, GitHubRequest.defaultOptions, options, { headers: headers });
  ...
  getAll(target, result = [], callback = null) {
    ...
    target = GitHubRequest.ensureMaxPerPage(target);
    const self = this;
    self.get(target, (err, response, body) => {
      ...
      if (response.headers.link) {
        ...
        const links = GitHubRequest.parseLinkHeader(response.headers.link);
        if (links.next) {
  ...
  get(target, callback = null) {
    ...
    request.get(target, self.options, (err, response, body) => {
      ...
      let toSleep = 0;
      if (remaining < self.options.tokenLowerBound && reset > 0) {
        toSleep = Math.max(reset - Time.now.to_i + 2, 0);
      }
