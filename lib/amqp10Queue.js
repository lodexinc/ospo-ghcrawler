  ...
  push(requests) {
    ...
    return Q.all(requests.map(qlimit(this.options.parallelPush || 1)(request => {
      ...
      try {
        ...
        this.logger.error(`Message could not be sent for ${this.queueName}. Error: ${error}`);
  ...
  pop() {
    ...
    if (request && request._message) {
      ...
      try {
        ...
        this.logger.error(`Message could not be ACKed for ${this.queueName}. Error: ${error}`);
        return Q.reject(error);
      }
    }
    return Q();
  ...
  abandon(request) {
    if (request && request._message) {
      ...
      try {
        ...
        this.logger.error(`Message could not be NAKed for ${this.queueName}. Error: ${error}`);
        return Q.reject(error);
      }
    }
    return Q();
