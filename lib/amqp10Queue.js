...
class Amqp10Queue {
  ...
  subscribe() {
    ...
    policy = AmqpPolicy.merge({ senderLink: { attach: { maxMessageSize: 50 * 1024 } }, receiverLink: { attach: { maxMessageSize: 50 * 1024 } } }, policy);
    ...
    return this.client.connect(this.url).then(() => {
      return Q.spread([
        ...
        receiver.on('errorReceived', err => {
          if (err.message && err.message.includes('connection was inactive')) {
            this.logger.info(`${this.queueName} - receiver error: ${err}`);
          } else {
            this.logger.error(err, `${this.queueName} - receiver error`);
          }
        });
        sender.on('errorReceived', err => {
          if (err.message && err.message.includes('IdleTimerExpired')) {
            this.logger.info(`${this.queueName} - sender error: ${err}`);
          } else {
