...
class Amqp10Queue {
  ...
  _release(message, delay = 0) {
    try {
      ...
      this.logger.info(`Could not release message for ${this.queueName}. Delaying ${delay} Error: ${error.message}`);
      if (delay) {
  ...
  _accept(message, delay = 0) {
    try {
      ...
      this.logger.info(`Could not accept message for ${this.queueName}. Delaying ${delay}. Error: ${error.message}`);
      if (delay) {
        return Q.delay(delay).then(() => this.receiver.accept(message));
      }
      return Q();
    }
  }
