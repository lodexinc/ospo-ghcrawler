...
class Amqp10Queue {
  ...
  pop() {
    ...
    while (message && moment().isAfter(message.messageAnnotations['x-opt-locked-until'])) {
  ...
  defer(request) {
    // do nothing.  Let the message fall on the floor.  It will eventually expire and get redelivered
  }
