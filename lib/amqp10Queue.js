...
class Amqp10Queue {
  constructor(client, name, formatter, manager, options) {
    this.client = client;
    this.name = name;
  ...
  subscribe() {
    ...
    return this.client.then(client => {
      this.manager.createQueue(this.queueName).then(() => {
  ...
  flush() {
    if (!this.manager) {
      return Q.reject(`Queue ${this.queueName} cannot be flushed. No queue manager`);
    }
    return Q
