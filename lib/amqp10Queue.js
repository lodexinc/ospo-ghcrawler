...
class Amqp10Queue {
  ...
  subscribe() {
    ...
    return this.client.connect(this.url).then(() => {
      return Q.spread([
        ...
        receiver.on('creditChange', () => {
          this.logger.info(`AMQP 1.0 credit changed to ${this.currentAmqpCredit}`);
        });
  ...
  _reconfigure(patches) {
    if (patches.some(patch => patch.path === '/credit') && this.currentAmqpCredit !== this.options.credit) {
