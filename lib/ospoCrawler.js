...
//        writeName:
  ...
  static createReprocessorService(options = null, logger = null) {
    options = options || OspoCrawler.createOptions();
    options.storage.writeName = config.get('GHCRAWLER_STORAGE_NAME') + '-new';
    return OspoCrawler.createCrawlerService(options, logger);
  }
  ...
  static createCrawler(options) {
    const queues = OspoCrawler.createQueues(options.queuing);
    const store = OspoCrawler.createStore(options.storage);
    const locker = OspoCrawler.createLocker(options.locker);
    const requestorInstance = OspoCrawler.createRequestor(options.requestor);
    const result = new Crawler(queues, store, locker, requestorInstance, options.crawler);
  ...
  static createAzureStorageStore(options) {
    return new AzureStorageDocStore(config.get('GHCRAWLER_STORAGE_ACCOUNT'), config.get('GHCRAWLER_STORAGE_KEY'), config.get('GHCRAWLER_STORAGE_NAME'), options);
  }
