...
class OspoCrawler {
  ...
  static createOptions() {
    ...
    return {
      crawler: {
        name: `${env}:${config.get('GHCRAWLER_CRAWLER_NAME') || 'crawler'}`,
        count: 0,
        pollingDelay: 5000,
  ...
  static createService(name) {
    const inMemory = name.toLowerCase().includes('inmemory') || config.get('GHCRAWLER_OPTIONS_PROVIDER') === 'InMemory';
    const crawlerPromise = OspoCrawler.createRefreshingOptions(OspoCrawler.createOptions(), inMemory).then(options => {
  ...
  static createStandardCrawler(options) {
    const logger = OspoCrawler.createLogger(false, true, 'info');
  ...
  static createStandardWithoutEventsCrawler(options) {
    options.queuing.events.weight = 0;
  ...
  static createInMemoryCrawler(options) {
    const logger = OspoCrawler.createLogger(true, true, 'info');
    options.queuing.events.weight = 0;
    OspoCrawler._addLoggerToOptions(options, logger);
  ...
  static createRefreshingOptions(options, inMemory = false) {
    const crawlerName = options.crawler.name;
    const promises = Object.getOwnPropertyNames(options).map(subsystemName => {
      const config = inMemory ? OspoCrawler.createInMemoryRefreshingConfig() : OspoCrawler.createRedisRefreshingConfig(crawlerName, subsystemName);
      return config.getAll().then(values => {
        if (Object.getOwnPropertyNames(values).length > 1) {
          return Q(values);
        }
        const subsystemOptions = options[subsystemName];
        return Q.all(Object.getOwnPropertyNames(subsystemOptions).map(optionName => {
          if (optionName === '_emitter') {
            return Q();
          } else {
            return config.set(optionName, subsystemOptions[optionName]);
          }
        })).then(() => { return config.getAll(); });
      });
    });
    return Q.all(promises).then(subsystemOptions => {
      const result = {};
      const names = Object.getOwnPropertyNames(options);
      for (let i = 0; i < names.length; i++) {
        result[names[i]] = subsystemOptions[i];
      }
      return result;
    });
  }
  ...
  static createRedisRefreshingConfig(crawlerName, subsystemName) {
    const redisClient = OspoCrawler.getRedisClient();
  ...
  static createInMemoryRefreshingConfig() {
