...
class OspoCrawler {
  ...
  static createAmqp10Queues(options) {
    ...
    const env = process.env.NODE_ENV;
    const tracker = OspoCrawler.createRequestTracker(`${env}:AMQP10:${options.topic}`, options);
  ...
  static createEventQueue(url, name, options) {
    ...
    const repoEvents = new Set(options.repoEvents || ['issues', 'issue_comment', 'push', 'status']);
    const orgEvents = new Set(options.orgEvents || ['membership']);
    ...
    return ServiceBusSubscription(serviceBusUrl, 'webhookevents', name, formatter);
  }
