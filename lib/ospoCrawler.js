...
class OspoCrawler {
  ...
  static createService(name) {
    const optionsProvider = config.get('CRAWLER_OPTIONS_PROVIDER') || 'redis';
  ...
  static createGitHubFetcher(store, options) {
    ...
    const limiter = OspoCrawler.createFetchLimiter(options);
  ...
  // static createFetchLimiter2(options) {
  //   const redisClient = options.limitStore === 'redis' ? OspoCrawler.getRedisClient(options.logger) : null;
  //   return new OperationLimiter(redisClient, options);
  // }

  // static createFetchLimiter1(options) {
  //   const redisClient = options.limitStore === 'redis' ? OspoCrawler.getRedisClient(options.logger) : null;
  //   const limiter = RateLimiter({
  //     redis: redisClient,
  //     interval: 1000,
  //     maxInInterval: options.rate
  //   });
  //   options._config.on('changed', () => {
  //     limiter.maxInInterval = options.rate;
  //   });
  //   return new OperationLimiter(limiter, options);
  // }
