...
class OspoCrawler {
  ...
  static createStandardCrawler(options) {
    ...
    OspoCrawler._enableEvents(options, OspoCrawler.getDefaultOptions().queuing.events.weight || 10);
    return OspoCrawler._createCrawler(options);
  }

  static createStandardWithoutEventsCrawler(options) {
    factoryLogger.info(`creating Without Events Crawler Started`);
  ...
  static _enableEvents(options, weight) {
    factoryLogger.info(`enabling events`, { queueing: options.queueing, weight: weight});
  ...
  static createCrawler(
    options, { queues = null, store = null, locker = null, fetcher = null, processor = null} = {}) {
  ...
  static createAzureStorageStore(options, name = null) {
    factoryLogger.info(`creating azure storage store`, {role: options.role});
  ...
  static getStorageRedisClient(options = {}) {
    const role = options.role ? `.${options.role}` : '';
    factoryLogger.info('retrieving storage redis client', { role: role });
    ...
    const tls = (config.get(`CRAWLER_STORAGE_REDIS_TLS${role}`) || config.get('CRAWLER_STORAGE_REDIS_TLS') || config.get('CRAWLER_REDIS_TLS')) === 'true';

    const redisClient = OspoCrawler.createRedisClient(url, key, port, tls, OspoCrawler.createLogger(true));
  ...
  static createAmqp10Queue(factory, name, tracker, options, receive = true) {
    ...
    const queue = factory.createQueueClient(name, formatter, options);
