...
const FileProcessor = require('ghcrawler').fileProcessor;
...
class OspoCrawler {
  ...
  static createInMemoryService(options = null, logger = null) {
    ...
    const processor = [new FileProcessor(store, ['msmetadata.yaml']), new GitHubProcessor(store)];
    const queues = OspoCrawler.createMemoryQueues(options.queuing);
    const locker = OspoCrawler.createNolock();

    const crawler = OspoCrawler.createCrawler(options, { store: store, queues: queues, locker: locker, processor: processor });
    return new CrawlerService(crawler, options);
  }
