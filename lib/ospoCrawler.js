...
class OspoCrawler {
  ...
  static createAmqpQueues(options) {
    ...
    return OspoCrawler.createQueueSet(manager, tracker, options);
  }
  ...
  static createMemoryQueues(options) {
    const manager = {
      createQueueChain: (name, tracker, options) => {
        return OspoCrawler.createMemoryQueue(name, options);
      }
    };
    return OspoCrawler.createQueueSet(manager, null, options);
  }
  ...
  static createQueueSet(manager, tracker, options) {
    ...
    const queues = OspoCrawler.addEventQueue(manager, [immediate, soon, normal, later], options);
  ...
  static addEventQueue(manager, queues, options) {
    if (options.events.provider && options.events.provider !== 'none') {
      queues.unshift(OspoCrawler.createEventQueue(manager, options));
    }
    return queues;
  }
