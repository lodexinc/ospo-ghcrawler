...
class OspoCrawler {
  ...
  static createReprocessorService(options = null, logger = null) {
    ...
    const newBaseStore = OspoCrawler.createRedisAndStorageStore(options.storage, newName);
    const newStore = OspoCrawler.createLoggingStore(newBaseStore, newName);
  ...
  static createGitHubFetcher(store, options) {
    ...
    const tokenFactory = OspoCrawler.createTokenFactory([config.get('GHCRAWLER_GITHUB_TOKEN')], options);
    return new GitHubFetcher(requestor, store, tokenFactory, options);
  }

  static createTokenFactory(tokens) {
    return {
      getToken: () => { return tokens[0]; }
    };
  }
  ...
  static createRedisAndStorageStore(options, name = config.get('GHCRAWLER_STORAGE_NAME')) {
    const blobService = OspoCrawler.getBlobService(config.get('GHCRAWLER_STORAGE_ACCOUNT'), config.get('GHCRAWLER_STORAGE_KEY'));
    const baseStore = new AzureStorageDocStore(blobService, name, options);
    return new UrlToUrnMappingStore(baseStore, OspoCrawler.getRedisClient(), name, options);
  }
  ...
  static createLoggingStore(baseStore, name = config.get('GHCRAWLER_STORAGE_NAME'), options) {
    const blobService = OspoCrawler.getBlobService(config.get('LOGS_STORAGE_ACCOUNT'), config.get('LOGS_STORAGE_KEY'));
  ...
  static getBlobService(account, key) {
