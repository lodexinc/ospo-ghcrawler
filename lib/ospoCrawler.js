...
class OspoCrawler {
  ...
  static createReprocessor2Service(options = null, logger = null) {
    ...
    const readOptions = Object.assign({}, options.storage, { role: 'read' });
    const readStore = OspoCrawler.createRedisAndStorageStore(readOptions);

    const writeStore = OspoCrawler.createRedisAndStorageStore(options.storage);
    const multiStore = new MultiStore(readStore, writeStore, options);
    const crawler = OspoCrawler.createCrawler(options, { store: multiStore });
    return new CrawlerService(crawler, options);
  }
  ...
  static createRedisAndStorageStore(options, name = null) {
    ...
    return new UrlToUrnMappingStore(baseStore, OspoCrawler.getRedisClient(options), baseStore.name, options);
  }
  ...
  static createLoggingStore(baseStore, name = config.get('LOGS_STORAGE_NAME'), options) {
    const blobService = OspoCrawler.createBlobService(config.get('LOGS_STORAGE_ACCOUNT'), config.get('LOGS_STORAGE_KEY'));
    return new LoggingStore(baseStore, blobService, name, options);
  }

  static getStorageRedisClient(options = {}) {
    const role = options.role || 'storage';
