...
const OperationLimiter = require('./operationLimiter');
...
class OspoCrawler {
  ...
  static createService(name) {
    ...
    const optionsProvider = config.get('CRAWLER_OPTIONS_PROVIDER') || 'defaults';
  ...
  static createGitHubFetcher(store, options) {
    ...
    const limiter = OspoCrawler.createRateLimiter(options);
  ...
  static createTokenFactory(options) {
    ...
    const factory = TokenFactory(tokens, options);
  ...
  static createTokenLimiter(options) {
    return options.limitStore === 'redis'
  ...
  static createRedisTokenLimiter(redisClient, options) {
    ...
    return RedisRateLimiter.create({
      redis: redisClient,
      key: request => ip + request.key,
      window: () => options.capWindow || 1,
      limit: () => options.capRate
    });
  }
  ...
  static createRateLimiter(options) {
    const counter = options.rateStore === 'redis'
      ? OspoCrawler.createRedisRateLimiter(getRedisClient(options.logger), options)
      : OspoCrawler.createInMemoryRateLimiter(options);
    options.rateUpdater = OspoCrawler._updateBaseline;
    return new TimeLimiter(limiter, options);
  }

  static _updateBaseline() {
    return Q.allSettled([0, 1, 2, 3].map(number => { Q(number * 100); })).then(times => {
      let total = 0;
      let count = 0;
      for (let time in times) {
        if (time.state === 'fullfilled') {
          total += time.value;
          count++;
        }
      }
  ...
  static createRedisRateLimiter(redisClient, options) {
    const ip = '';
    return RedisRateLimiter.create({
      redis: redisClient,
      key: request => ip + request.key,
      window: () => options.rateWindow || 15000,
      limit: () => options.rate || 15000
    });
  }

  static createInMemoryRateLimiter(options) {
    return InMemoryRateLimiter.create({
      key: request => request.key,
      window: () => options.rateWindow || 15000,
      limit: () => options.rate || 15000
  ...
  static createStore(options) {
    ...
    switch (options.provider) {
      ...
      case 'mongo': {
        return OspoCrawler.createMongoDocStore(options);
      }
      case 'memory': {
