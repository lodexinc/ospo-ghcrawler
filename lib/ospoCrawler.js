...
class OspoCrawler {
  ...
  static createStandardWithoutEventsService(options = null, logger = null) {
    options = options || OspoCrawler.createOptions();
    options.queuing.events.weight = 0;
    return OspoCrawler.createStandardService(options, logger);
  }
  ...
  static createAmqpQueues(options) {
    ...
    const queues = OspoCrawler.addEventQueue([immediate, soon, normal, later], options);
    const queuesMetrics = OspoCrawler.createQueuesMetrics(options.queueName);
    return new QueueSet(queues, deadletter, options, queuesMetrics);
  }
  ...
  static createQueuesMetrics(queueNamePrefix = '') {
    const metrics = new RedisMetrics({ client: OspoCrawler.getMetricsRedisClient() });
    const queueNames = ['immediate', 'soon', 'normal', 'later', 'deadletter'];
    ...
    queueNames.forEach(queueName => {
      ...
      operations.forEach(operation => {
        ...
        queuesMetrics[queueName][operation] = metrics.counter(name, { timeGranularity: 'second', namespace: 'crawlermetrics', expiration: { second: 3600 } }); // Stored in Redis as {namespace}:{name}:{period}
      });
    });
