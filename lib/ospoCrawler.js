...
class OspoCrawler {
  ...
  static createEventQueue(options) {
    ...
    const topic = config.get('GHCRAWLER_EVENT_TOPIC_NAME');
    const name = config.get('GHCRAWLER_EVENT_QUEUE_NAME');
    return OspoCrawler.createEventQueue(url, topic, name, options);
  }

  static addEventQueue(queues, options) {
    if (options.events.weight) {
      options.weights.unshift(options.events.weight);
      queue.unshift(OspoCrawler.createEventQueue(options));
    }
    return queues;
  }
...
class EventFormatter {
  constructor(options) {
    ...
    this.repoEvents = new Set(options.events.repoEvents || ['issues', 'issue_comment', 'push', 'status']);
    this.orgEvents = new Set(options.events.orgEvents || ['membership']);
  }
