...
class OspoCrawler {
  ...
  static createFetcherMetrics(crawlerName, options) {
    const metrics = new RedisMetrics({ client: OspoCrawler.getRedisClient() });
    const names = ['fetch'];
    const result = {};
    names.forEach(name => {
  ...
  static getStorageRedisClient(options = {}) {
    ...
    const redisClient = OspoCrawler.createRedisClient(url, key, port);
  ...
  static createRedisClient(url, key, port) {
    const options = { auth_pass: key, tls: { servername: url }};
    const redisClient = redis.createClient(port, url, options);
    redisClient.on('error', error => appInsights.client.trackException(error, { name: 'CrawlerRedisError' }));
    redisClient.on('reconnecting', properties => appInsights.client.trackEvent('CrawlerRedisReconnecting', properties));
    setInterval(() => {
      redisClient.ping((err, result) => {
        if (err) { appInsights.client.trackException(err, { name: 'CrawlerRedisPingFailure' }); }
      });
    }, 60 * 1000);
    return redisClient;
  }
