...
class RateLimitedPushQueue extends NestedQueue {
  ...
  _pushOne(request) {
    debug(`_pushOne(${request.meta.loopName}:${request.toUniqueString()}: enter`);
    const deferred = Q.defer();
    const self = this;
    this.limiter(null, (error, rate) => {
      if (error) {
        debug(`_pushOne(${request.meta.loopName}:${request.toUniqueString()}: exit (error)`);
        return deferred.reject(error);
      }
      if (rate.over) {
        return deferred.resolve(Q.delay(Math.floor((this.options.pushRateWindow || 2) * 1000 / 4)).then(() => {
          debug(`_pushOne(${request.meta.loopName}:${request.toUniqueString()}: exit (delayed)`);
          return self._pushOne(request);
        }));
      }
      debug(`_pushOne(${request.meta.loopName}:${request.toUniqueString()}: exit (success)`);
