...
class RedisRequestFlagger {
  constructor(prefix, redisClient, logger) {
    this.prefix = prefix;
  ...
  setFlag(request) {
    ...
    this.redisClient.set([key, value, 'EX', '100', 'NX'], function (err, reply) {
      if (err) {
  ...
  _getKey(request) {
    ...
    return `${env}:${this.queueName}:${request.url}:force:${!!request.force}:${this.prefix}`;
  }
