...
class RedisRequestTracker {
  ...
  track(request, operation) {
    debug(`track(${request.meta.loopName}:${request.toUniqueString()}): enter`);
    ...
    return this._lockExecuteUnlock(key + '-lock', () => {
      return self._getTag(key, request).then(timestamp => {
        if (timestamp) {
          ...
          this.logger.verbose(`Bounced ${attemptString}${request.type}@${request.url} from ${diff}ms ago`, request.meta);
          request.queueOutcome = 'Bounced';
          debug(`track(${request.meta.loopName}:${request.toUniqueString()}): exit (bounced)`);
          return Q();
        }
        return operation(request).then(result => {
          debug(`track(${request.meta.loopName}:${request.toUniqueString()}): operation success`);
          return self._setTag(key, request)
            .then(success => {
              debug(`track(${request.meta.loopName}:${request.toUniqueString()}): exit (success)`);
              return result;
            });
        });
      });
  ...
  untrack(request) {
    debug(`untrack(${request.meta.loopName}:${request.toUniqueString()}): enter`);
    const key = this._getKey(request);
    const self = this;
    return this._lockExecuteUnlock(key + '-lock', () => {
      return self._removeTag(key, request);
    }).then(request => {
      debug(`untrack(${request.meta.loopName}:${request.toUniqueString()}): exit (success)`);
      return request;
    });
  }
