...
class AzureStorageDocStore {
  constructor(account, key, name, options) {
    this.account = account;
    this.key = key;
    this.name = name;
    this.writeName = options.writeName || name;
  ...
  upsert(document) {
    ...
    this.service.createBlockBlobFromText(this.writeName, blobName, text, options, (error, result, response) => {
      if (error) {
        return deferred.reject(error);
      }
  ...
  get(type, url) {
    ...
    this.service.getBlobToText(this.name, blobName, (error, text, blob, response) => {
      ...
      memoryCache.put(url, { etag: result._metadata.etag, document: result }, this.options.ttl);
      deferred.resolve(result);
    });
    return deferred.promise;
  }
